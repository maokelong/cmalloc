...
void global_pool_init(void) {
	...
	int i;
	GLOBAL_POOL.meta_pool.pool_start = require_vm(STARTA_ADDR_META_POOL, LENGTH_META_POOL);
	...
	GLOBAL_POOL.data_pool.pool_start = require_vm(STARTA_ADDR_DATA_POOL, LENGTH_DATA_POOL);
...
void thread_local_heap_deallocate_data_block(thread_local_heap *tlh,
	...
	void *correlated_shadow_block =
		(void*)smb + sizeof(super_meta_block) + sizeof(shadow_block) * sn_data_block;

	// recycle the correlated shadow block
	seq_enqueue(&smb->freed_blocks, correlated_shadow_block);

	// check target smb's life cycle
	...
	// TODO: put the extra cold super blocks in the global pool
}
